...
function buscarTodo(req, res) {
  palapaModel
    ...
    .catch((e) => {
      return res
        .status(404)
        .send({ mensaje: `Error al solicitar la información ${e}` });
    });
}

function agregar(req, res) {
  // console.log(req.body)
  new palapaModel(req.body)
    .save() //despues de guardar el objeto, se retorna una promesa
    .then((info) => {
...
function buscarBebida(req, res, next) {
  //Simepre debe de llevar una propiedad y atributos
  if (!req.body) req.bodi = {};
  let consulta = {};
...
function mostrarBebida(req, res) {
  if (req.body.e)
    return (
      res.status(404), send({ mensaje: "Error al consultar la informacion" })
    );
  if (!req.body.bebidas)
...
function actualizarBebida(req, res) {
  ...
  palapaModel
    .findOneAndUpdate(filtro, datosActualizar, {
      new: true,
      runValidators: true,
    })
    .then((bebidaActualizada) => {
      if (!bebidaActualizada) {
        return res
          .status(404)
          .send({ mensaje: "No se encontro ninguna bebida" });
      }

      return res.status(200).send({
        mensaje: "La bebida se actualizo correctamente",
    ...
    .catch((e) => {
      return res
        .status(500)
        .send({ mensaje: `Error al actualizar la bebida: ${e}` });
    });
}
